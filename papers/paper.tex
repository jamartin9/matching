...
\begin{abstract}
%\boldmath
We implemented a parallel algorithm for finding a stable marriage on massively
parallel graphics processing units (GPUs). The algorithm is the Parallel
Iterative Improvement (PII) Algorithm for the Stable Matching Problem proposed
by Lu and Zheng in 2003 \cite{lu-zheng}.  The algorithm has applications in
real-time communication switching networks \cite{lu}.  Global memory access for
the many execution units on GPUs produces a speedup over what was possible on
previously used achitectures.
\end{abstract}
...
\begin{IEEEkeywords}
parallel prcoessing, stable matching, stable marriage, graphics processing
units, computer networks.
\end{IEEEkeywords}
...
\IEEEPARstart{T}{he} stable marriage problem is often similarly stated as
...
\begin{figure}[!t]
\centering
\includegraphics[angle=270,scale=0.35]{results}
